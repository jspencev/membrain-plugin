...
name: ask
description: >-
  Ask the user's braindump (their personal permanent memory) a question and
  relay the answer — a thin pass-through to the braindump MCP ask tool. Use
  when the user runs /braindump:ask <question> or wants a direct answer from
...
Call the braindump MCP `ask` tool with the user's arguments as the question,

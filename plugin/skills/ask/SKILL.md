...
name: ask
description: >-
  Ask the user's tilden (their personal permanent memory) a question and
  relay the answer — a thin pass-through to the tilden MCP ask tool. Use
  when the user runs /tilden:ask <question> or wants a direct answer from
...
Call the tilden MCP `ask` tool with the user's arguments as the question,
